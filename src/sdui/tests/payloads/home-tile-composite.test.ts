...
const raw = JSON.parse(
  fs.readFileSync(path.join(__dirname, '../../../../payloads/home-tile-composite.json'), 'utf-8')
);

describe('payloads/home-tile-composite.json', () => {
  ...
  test('every tile section (1, 2, 3, 4, 6) is composed of "tile" nodes, not stack{image,text}', () => {
    const payload = raw as Payload;
    const tileSectionIds = ['home.buyCar', 'home.sellCar', 'home.loans', 'home.services', 'home.manageVehicle'];
    const tileTypesFound: string[] = [];

    function walk(node: SDUINodeData) {
      if (node.id.includes('.tile.')) tileTypesFound.push(node.type);
      node.children?.forEach(walk);
    }
    payload.sections.filter((s) => tileSectionIds.includes(s.id)).forEach(walk);

    expect(tileTypesFound.length).toBeGreaterThan(0);
    expect(tileTypesFound.every((t) => t === 'tile')).toBe(true);
  });
  ...
  test('a sampled tile (Hatchback) renders its label and is tappable', async () => {
    const payload = raw as Payload;
    await render(React.createElement(SDUIScreen, { payload }));
    expect(screen.getByText('Hatchback')).toBeTruthy();
    expect(screen.getByTestId('home.buyCar.tile.hatchback')).toBeTruthy();
  });
});
